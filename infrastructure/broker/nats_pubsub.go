...
import (
	...
	"github.com/AniFapper/go-common-ocherk/contracts"
...
func NewNatsPubSub[T any](nc *nats.Conn, codec contracts.Marshaller) *NatsPubSub[T] {
	return &NatsPubSub[T]{
		nc:    nc,
		codec: codec,
	}
...
func (p *NatsPubSub[T]) Subscribe(ctx context.Context, topic string, handler contracts.Handler[T], opts ...contracts.SubscribeOption) (contracts.Unsubscriber, error) {
	// Initialize default options and apply functional modifiers
	options := &contracts.SubscribeOptions{}
